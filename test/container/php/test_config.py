...
@pytest.mark.php_cli
def test_cli_config_file_are_loaded(host):
    assert host.file(CONFIG_DIR + '/conf.d/020-cli.ini').exists is True

@pytest.mark.php
def test_config_files_are_loaded(host):
    files = get_loaded_config_files(host)
    assert '/usr/local/etc/php/php.ini' in files
    assert '/usr/local/etc/php/conf.d/000-default.ini' in files
    assert '/usr/local/etc/php/conf.d/050-uploads.ini' in files
